...
from relspec.measure import (Trajectory, apply_plan, cross_plan,  # noqa: E402
                             detect_emergence, law_plan)


def gt_embedding(world, rng=None):
    E = np.array([world.meta["gt_ent"][e] for e in world.entities]
...
def main():
    ...
    for tag, E in (("ground truth", gt_embedding(w)),
        ...
        print("%-20s %-13s resolved %5.1f%% | offset error %.2e"
    ...
    print("criterion          retrieval 100%% + geometry 3.00 -> t* %s (want nan)"
          % t.emergence(S)["L"])
    print("criterion          retrieval 100%% + geometry 0.01 -> t* %s (want 0.0)"
          % u.emergence(S)["L"])
