...
def main():
    ...
    print("   %d tokens, %d facts, z-facts %s, null dim %d -> %d"
          % (a.P, len(a.facts), zc, nulldim(a, sa), nulldim(b, sb)))
    print("   rho before: %s" % {l.name: round(sa.identifiability(l, S)["rho"], 4)
                                 for l in a.laws})
    print("   rho after:  %s" % {l.name: round(sb.identifiability(l, S)["rho"], 4)
                                 for l in b.laws})
    print("   held-out composites per law: %s"
          % {k: len(v) for k, v in hi.items()})
