...
def main():
    ...
    obs = train.train(models.make_model(w, DEPTH, S), s, lr, EPOCHS, S,
                      order_seed=7, eval_every=EVERY, plan=plan)
    pre, _ = theory.predict(s, DEPTH, lr, EPOCHS, models.make_model(w, DEPTH, S),
                            settings=S, eval_every=EVERY, plan=plan)

    for lab, t in (("network", obs), ("prediction", pre)):
        r, g = t.retrieval["cross"], t.geometric["cross"]
        print("%-11s %d evaluations | resolved %5.1f%% -> %5.1f%% | "
              "geometric %.3f -> %.2e | t* %s"
              % (lab, len(t.epochs), r[0], r[-1], g[0], g[-1],
                 t.emergence(S)["cross"]))
    ...
    ratio = np.median(np.abs(np.log10(np.maximum(obs.geometric["cross"], 1e-16)
                                      / np.maximum(pre.geometric["cross"], 1e-16))))
    print("network vs prediction: max retrieval gap %.1f points | "
