"""Figure S1: full-run event times for all 16 laws in the first 200 worlds.

The appendix figure: every law-world's predicted and observed emergence
epoch, and the distribution of emergence times across worlds for each law,
over the complete Experiment 1 runs rather than the early window Figure 1
draws.

  usage:  python figS1.py                  from the records
          python figS1.py --from-summary   redraw from the stored summary
"""
import json
...
def main(from_summary=False):
    ...
    for extension in ('pdf', 'png'):
        output = Path(_paths.FIGURES, 'figS1_timing.' + extension)
