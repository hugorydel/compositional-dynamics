...
not usable as evidence.
  ...
  usage:  python controls.py --cells f3:0:1 f3:0:2 f3:0:3 f2:0:1:A
          python controls.py --plot
"""
...
def out_dir(sub):
    """Controls are written beside the experiment they belong to."""
    return Path(RESULTS) / ("f2" if sub == "f2" else "f3")
...
def done(spec):
    ...
    tag = "w%02d_d%d" % (seed, depth) + ("_%s" % closed if sub == "f2" else "")
    path = out_dir(sub) / ("controls_%s.json" % tag)
    if not path.exists():
        return False
    try:
...
def load(kind, stem):
    with np.load(out_dir(kind) / (stem + ".npz")) as z:
...
def plot(seed=0):
    ...
    for kind, title in (("f3", "Experiment 3"), ("f2", "Experiment 2")):
        ...
        for col, depth in enumerate((1, 2, 3)):
            stem = ("controls_w%02d_d%d" % (seed, depth) if kind == "f3"
                    else "controls_w%02d_d%d_A" % (seed, depth))
            a = load(kind, stem)
            meta = json.loads((out_dir(kind) / (stem + ".json")).read_text())
            ep = a["epochs"]
            m = ep > 0
            if kind == "f3":
        ...
        path = out_dir(kind) / "diagnostic_checks.png"
